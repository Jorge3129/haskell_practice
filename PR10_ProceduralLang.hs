...
updateValue a b ps =
  case elemIndex a (map fst ps) of
    Nothing  -> ps ++ [(a, b)]
...
evStmt (AssignA nm ix ex) dfx _ st = case lookup nm st of
  ...
  (Just arr) -> updateValue nm newArr st
    where
      newArr = updateArray arr (evExp ix dfx st) (evExp ex dfx st)
...
evStmt (Call nm args) dfx dpx st = case lookup nm dpx of
  ...
  (Just (as, stmt)) ->
    let vs = evArgs args dfx st
     ...
     in [var | var@(vnm, _) <- resSt, vnm `notElem` varNms]
evStmt (Block vars stmts) dfx dpx st =
  let varNms = map getVarName vars
   ...
   in [var | var@(nm, _) <- resSt, nm `notElem` varNms]
...
iswfExp (FunApp nm args) ve fe = do
  ...
  let argTs = map (\x -> iswfExp x ve fe) args
      zipArgs = zipWith (\a b -> Just b == a) argTs params
  if length argTs == length params && and zipArgs
    then Just It
    else Nothing

-- Задача 7 ------------------------------------
iswfStmt :: Stmt -> VarEnv -> FunEnv -> ProcEnv -> Bool
iswfStmt (Assign nm ex) ve fe _ =
  Just True
    == ( do
           tv <- lookup nm ve
           te <- iswfExp ex ve fe
...
iswfStmt (AssignA nm i ex) ve fe _ =
  Just True
    == ( do
           tv <- lookup nm ve
           ti <- iswfExp i ve fe
...
iswfFunDef (_, (vars, body)) fe =
  uniq (map fst varsEnv)
    && Just It == iswfExp body varsEnv fe
  where
    varsEnv = map varDefToEnv vars
...
lookUp a abx = Data.Maybe.fromMaybe (error "lookUp") (lookup a abx)
