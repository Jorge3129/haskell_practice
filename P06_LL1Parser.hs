...
parse gr ctl st = 
  let start = (fst . head) gr
      input = st ++ "$"
      predicate = (\(_, stack, res) -> null stack || res == Nothing)
...
step gr ctl (input, stack, result) =
  let top = head stack
      ch = head input
  in if isNonTerm top then 
    let pIx = [ b | (a, b) <- ctl, a == (top, ch)]
    in if null pIx then (input, stack, Nothing) else 
       let (_, out) = gr !! (head pIx)
       in (input, out ++ (tail stack), maybe Nothing (\xs -> Just (xs ++ [head pIx])) result) 
    else if top == ch 
...
first pFst st =
  if null st then "$" else
  if (isTerm . head) st then (head st:[]) else
...
testingLL1 gr pFst pNxt = all (\(n, rls) -> 
