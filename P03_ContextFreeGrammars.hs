...
isGrammar = foldl (\res p -> res && isNonTerm (fst p)) True
...
newMinN ps = head [x | x <- ['A' .. 'Z'], x `notElem` allNotT ps]
...
hasSomeElemsFrom xs ys = any (`elem` ys) xs
...
getAccsFromKnown ps knownAccs =
  [ x
    | x <-
        (nub . concat) [getRightNonTs p | p <- ps, fst p `elem` knownAccs],
      x `notElem` knownAccs
  ]
...
reduce ps =
  let gens = buildGen ps
      accs = buildAcc ps
   in [p | p <- ps, let n = fst p, n `elem` gens, n `elem` accs]
...
sndLeftRCondition x = any (\p -> fst p == x && not (fstLeftRCondition p))

-- Задача 4.b -----------------------------------------
deleteLeftR :: Grammar -> Char -> Grammar
deleteLeftR ps nonTerm =
  if nonTerm `notElem` findLeftR ps
    then ps
    else
...
refactProd nonTerm newNonTerm p
  | fst p /= nonTerm = p
  | fstLeftRCondition p = (newNonTerm, tail (snd p) ++ [newNonTerm])
  | otherwise = (fst p, snd p ++ [newNonTerm])

restoreStartNonT :: Char -> Grammar -> Grammar
restoreStartNonT nonTerm gr =
  if fst (head gr) == nonTerm
    then gr
    else
...
isCommonPrefix xs ys =
  let zipped = zipWith (==) xs ys
   in (not (null zipped) && head zipped)
...
rmPrefix prefix = drop (length prefix)

-- Задача 6.a -----------------------------------------
isLeftDerivationS :: Grammar -> DerivationS -> Bool
isLeftDerivationS gr sx =
  foldl
    ( \res (start, end) ->
...
isLeftDerivationR gr ix =
  fst
    ( foldl
        ( \acc@(success, expr) ruleIndex ->
