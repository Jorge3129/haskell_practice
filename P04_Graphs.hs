...
stepP :: Graph -> [[[Int]]] -> [[[Int]]]
stepP gr pss@(psn:_) =
        [t:p | p@(x:xs) <- psn, notElem x xs, t <- gr !! x] : pss
...
noLoops :: Graph -> Bool
noLoops gr = null [ x | x <- zip [0 .. ] gr, elem (fst x) (snd x) ]
...
shortWay gr a b = if a == b then [] else
    let paths = [ y | x <- allPaths gr a, y <- (reverse x), head y == b]
    in if null paths
        then []
        else reverse (head (reverse paths))

-- Задача 5 ------------------------------------
isConnecting :: Graph -> Bool 
isConnecting gr = null [ p | p <- (uniquePairs . nodes) gr, 
                not (isConnectedTo gr (fst p) (snd p))]
...
components gr = reverse (foldl (\xss v -> 
